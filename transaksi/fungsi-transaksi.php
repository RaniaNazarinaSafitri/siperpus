...
function ambilAnggota($kon){
 ...
 function ambilPeminjaman($kon, $id_pinjam){
   $sql = "SELECT * FROM peminjaman p INNER JOIN anggota a ON p.id_anggota=a.id_anggota INNER JOIN detail_pinjam db USING(id_pinjam) INNER JOIN buku b ON dp.id_buku=b.id_buku WHERE id_pinjam = '$id_pinjam'" ;
   $res = mysqli_query($kon, $sql);
